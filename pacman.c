    ...
    while(c!=ESCAPE ) 
    {
        ...
        if(sw_phantom_trace){
             if ( Timer( &tp[i], time_limitp[i] )){
                 ...
                 At px[i],py[i]; clear_actor(); //draw_ascii_phantoms(BACKGROUND,BACKGROUND,i);
        ...
        /* analiza posicion de pacman*/
         if( Timer( &t, time_limit ) ){
             At x,y; clear_actor(); //draw_ascii_pacman(BACKGROUND,BACKGROUND,dir,boca);
...
void clear_actor(){
   static char actor[1024];
   sprintf(actor,"\x1b[%d;%dH     \x1b[%d;%dH     \x1b[%d;%dH     \x1b[%d;%dH     \x1b[%d;%dH     ",
                 SCREEN_ROW, SCREEN_COL,
                 SCREEN_ROW+1, SCREEN_COL,
                 SCREEN_ROW+2, SCREEN_COL,
                 SCREEN_ROW+3, SCREEN_COL,
                 SCREEN_ROW+4, SCREEN_COL);
...
void draw_ascii_pacman( int color, int bcolor,int pos, int boca )
{
    ...
    for (i=0; i<5; i++){
        ...
        int n = sprintf( psimb, "\x1b[38;5;%dm\x1b[%d;%dH%s\x1b[0m",
                         color,(SCREEN_ROW+i), SCREEN_COL, pacman[i][ (boca ? 0 : pos) ]);
        const char* pf = psimb;
...
void draw_ascii_phantoms( int color, int bcolor,int pos )
{
    ...
    for (i=0; i<5; i++){
        ...
        int n = sprintf( psimb, "\x1b[38;5;%dm\x1b[%d;%dH%s\x1b[0m",
                         color,(SCREEN_ROW+i), SCREEN_COL, phantoms[i][pos]);
        const char* pf = psimb;
        memcpy((void *) vid + cntbuff, pf, n );
        cntbuff = cntbuff + n;
    }
...
void print_dots( int colorF, int colorB ) 
{
   ...
   //cb=sprintf(points, "\x1b[38;5;%dm\x1b[48;5;%dm",colorF,colorB);
   cb=sprintf(points, "\x1b[38;5;%dm",colorF);
